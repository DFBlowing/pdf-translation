...
```
0. ask        → 先问你：只要 Markdown，还是要排版好的 PDF？正文/图片各要几栏？报出预估 token 与耗时
1. probe      → 这个 PDF 有文字层吗？哪些页需要 OCR？
1.5 truncation→ 源文件自己有没有丢字（代码框被裁切）？先量出来，再决定复原工作量
2. extract    → 逐页取出文本块 + 坐标 + 字体字号 + 颜色，图片按 300 dpi 裁出
3. classify   → 哪些片段是公式，哪些块是标题 / 代码 / 题注
4. translate  → 模型写中文，产出结构化的正文文件。
                先把原文的链接清单抽出来、写进每一批的写作说明（extract-source-links.py），
                写完再用 restore-links.py 把它们接回正文
5. verify     → 四道闸门，按顺序跑，任何一道不过就停下：
                  fidelity.py        有没有漏句、有没有把题注写成摘要
                  coverage-check.py  有没有整段源码漏译
                  links-audit.py     每条链接标注是否都有交代
                  check-markup.py    接链接有没有把 HTML 接坏
6. typeset    → HTML + KaTeX → headless Chrome → PDF，盖章页眉页脚，
                最后由 add-internal-links.py 把目录跳转写进成品 PDF
```
...
这是实际翻过车的一处：源文件里有 **251 条**链接标注，第一次交付只留下 67 条。原因不是找不到，而是**分工把上下文切开了** —— 前 30 页的样本是手写的、链接写在代码里，其余五个批次由并行的子 agent 按一份**从没提过链接**的写作说明产出，于是它们一个 `<a href>` 都没写。**没写进说明的事实，片段里就不会有。**

处置：站点页脚那三条每章重复的 URL（`github.com/cs231n`、`twitter`、`mailto:`，共 66 条）按「译文不是网站」丢掉，其余逐条接回正文；**另有约 27% 的链接定位不到**（可见文字已整体译成中文、不留拉丁痕迹，如 `in the documentation.` → 「在文档中」），这些**只报告、不猜** —— 接错的链接比缺失的链接更糟。

> 口径要写清楚：**251 条标注**（源文件实际有 253 条 URI 标注，其中 2 条与别的完全重复）= 66 条站点 chrome + 52 条本就在 + 66 条本轮接回 + 67 条无法定位。别用「出现过多少个不同 URL」代替这个账 —— 同一个 URL 出现一次就算「在」，三处引用同一篇论文会被并成一条。
...
| **Stanford CS231n 讲义**（纯文字 + 代码） | **191 页 / 50,529 词 / 82 图** | **222k**（实测值） | **约 29 分钟**（实测值） |
...
│   ├── detect-truncation.py        阶段 1.5 — 找出源文件被裁掉的代码行
│   ├── extract.py                  阶段 2 — structure.json + 300dpi 图片
│   ├── classify.py                 阶段 3 — 公式片段 + 标题层级（字号 + 颜色）
│   ├── extract-source-links.py     阶段 4 — 抽出原文的链接标注与锚文本
│   ├── restore-links.py            阶段 4 — 把内容链接接回译文正文
│   ├── fidelity.py                 阶段 5 — 漏译/未译/题注闸门
│   ├── coverage-check.py           阶段 5 — 源文到译文的整段覆盖（漏页）
│   ├── links-audit.py              阶段 5 — 链接覆盖率：源文标注 vs 译文
│   ├── check-markup.py             阶段 5 — 接完链接后的 HTML 完整性
│   ├── add-internal-links.py       阶段 6 — 给成品 PDF 加目录跳转
...
- **脚注无法贴页**（见 FAQ）。
- **页眉页脚是脚本盖的章**，不是原文版面。
- **约四分之一的链接会定位不到。** 可见文字被整体译成中文、不留拉丁痕迹的那些（`here.`、`in the documentation.` → 「在文档中」）无法用字符串匹配找到，按序数硬配又会整体错位一条。这些**只报告、不猜** —— 接错的链接比缺失的链接更糟。
- **源文件的截断只能从上游复原。** 代码框被 `overflow: hidden` 裁掉的字符不在 PDF 内容流里，提取工具取不到；上游找不到对应版本时，只能按语法谨慎补全，补不了的**原位保留**而不是编一个结尾。
...
| **Stanford CS231n 讲义** | 191 页 → 译文 155 页 · 82 图 · 133 条链接 | 13 个并行批次（实测约 29 分钟）、代码原样保留、被裁掉的代码从上游复原 |

文件较大（合计约 35 MB），所以**不放进仓库**，挂在 Release 上：

**[⬇ 下载两个案例](https://github.com/DFBlowing/pdf-translation/releases/tag/cases-v1)** · 细节见 [`docs/cases/`](docs/cases/README.md)

> CS231n 那份源文件本身有缺陷：代码框用了 `overflow: hidden`，检测器在源文件里报出 **73 行**代码贴着裁切边界（分布 42 页），**缺失的字符在 PDF 里根本不存在**，任何提取工具都拿不到。译文改从官方讲义**逐处复原**：实测 72 处候选里，65 处可机械证明已复原（输出在该处写下了源文件里没有的字符），其余是检测器的边界误报，逐条人工看过之后确认也全部已复原 —— **成品里没有一处「原文截断」标注**。
